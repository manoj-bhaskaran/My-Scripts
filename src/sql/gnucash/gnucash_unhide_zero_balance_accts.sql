...
a.parent_guid
        FROM accounts AS a
        INNER JOIN account_tree AS at ON a.parent_guid = at.guid
    ),
...
SUM(value_num) AS balance
    ...
    SELECT a.guid
    FROM account_tree AS a
    LEFT JOIN accounts AS children ON a.guid = children.parent_guid
    INNER JOIN zero_balance_accounts AS z ON a.guid = z.account_guid
