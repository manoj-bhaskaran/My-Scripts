...
:: ------------------------------------------------------------
:: Copy-FilesByExtension launcher - Version 1.0.0
:: - Prefers PowerShell 7 (pwsh.exe), falls back to Windows PS.
:: - Invokes Copy-FilesByExtension.ps1 with settings from .env.
:: - Shows a MessageBox on fatal failure; returns script exit code.
:: ------------------------------------------------------------
...
set "SCRIPT=%REPO_ROOT%\src\powershell\file-management\Copy-FilesByExtension.ps1"
...
call :LogInfo "Executing Copy-FilesByExtension.ps1"

:: Run the script (all config is read from .env)
...
if "%RC%"=="1" (
  ...
  "%RUNNER%" -NoLogo -NoProfile -Command ^
    ...
    "'The copy-files task failed. Check the log for details.'," ^
    "'CopyFilesByExtension Failed') | Out-Null"
)
