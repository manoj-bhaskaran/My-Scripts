...
class IdMetadataPrefetcher:
    ...
    def _classify_prefetched_id(self, fid: str, result: PrefetchResult) -> bool:
        if fid in self._id_prefetch_errors:
            ...
            if err == "HTTP 404":
                result.buckets.add(ValidationBucket.NOT_FOUND, fid)
            elif err == "HTTP 403":
                result.buckets.add(ValidationBucket.NO_ACCESS, fid)
            else:
                result.counters.transient_errors += 1
                result.counters.transient_ids.append(fid)
    ...
    def _fetch_and_handle_metadata(
        self, service, fid: str, fields: str, result: PrefetchResult
    ) -> None:
        data, error, status = self.parent._with_retries(
            lambda: self.parent._execute(service.files().get(fileId=fid, fields=fields)),
            terminal_statuses=(403, 404),
            logger=self.parent.logger,
            ctx=f"files.get(fileId={fid})",
        )
        if error is None:
            self._handle_prefetch_success(fid, data, result)
            return
    ...
    def prefetch_ids_metadata(self, fids: List[str]) -> PrefetchResult:
        service = self.parent.auth._get_service()
        fields = self.parent._id_discovery_fields()
        result = PrefetchResult()
        for fid in fids:
            if self._should_skip_invalid_id(fid, result):
                continue
            if self._classify_prefetched_id(fid, result):
                continue
    ...
    def emit_parity_metrics(self, result: PrefetchResult) -> bool:
        try:
            ...
            metrics = {
                ...
                "skipped_non_trashed": c.skipped_non_trashed,
                "errors": c.err_count,
                "seen": seen,
                "mismatch": mismatch,
            }
            ...
            if mismatch:
                self.parent.logger.warning(
                    ...
                    seen,
                    classified,
                    c.skipped_non_trashed,
                    c.err_count,
                )
            return mismatch
        except Exception as e:
            self.parent.logger.debug("Parity metrics emission failed: %s", e)
            return False
