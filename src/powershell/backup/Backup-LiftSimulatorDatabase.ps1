<#
 .VERSION
     1.0.0
.SYNOPSIS
    Runs a PostgreSQL backup for the lift_simulator database via the PostgresBackup module.

.DESCRIPTION
    - Ensures backup and log folders exist under:
        D:\pgbackup\lift_simulator\
        D:\pgbackup\lift_simulator\logs
...
.NOTES
    ...
    ## 1.0.0 - 2026-01-12
    ### Added
    - Initial implementation of lift_simulator database backup
    - Uses PowerShellLoggingFramework for standardized logging
    - Implements .pgpass authentication validation with ACL checks
    - Leverages PostgresBackup module for backup operations
    - Configurable retention policy (90 days, minimum 3 backups)

    Authentication uses .pgpass for backup_user.
    Requires:
      - PostgresBackup module deployed/available on PSModulePath
      - .pgpass (or equivalent) with entry for lift_simulator database
      - backup_user with sufficient privileges on lift_simulator database
...
function Invoke-BackupMain {
    ...
    param(
        ...
        [string]$Database = 'lift_simulator',
        [string]$BackupRoot = 'D:\pgbackup\lift_simulator',          # where .backup files go
        [string]$LogsRoot = 'D:\pgbackup\lift_simulator\logs',      # where .log files go
