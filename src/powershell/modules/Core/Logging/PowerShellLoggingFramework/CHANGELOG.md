...
- Public log-level constants API via `Get-LoggerLevelValue` (`DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`).
- Public log-target setter `Set-LoggerLogFilePath -Path <string>` to encapsulate runtime log file override scenarios.

### Changed

- `Initialize-Logger` now defaults `-LogLevel` using named level constants and validates numeric values against framework-supported levels.
