...
Optional. Minimum length of the generated file name. Defaults to 4.
Must be between 1 and 255 and <= MaximumLength.
...
Optional. Maximum attempts to regenerate when a reserved device name is produced.
Defaults to 100. On exhaustion, a safe first character is substituted to ensure
the result is not a reserved device name.
...
4. **Reserved Name Check**:
   ...
   - Respects `-MaxAttempts`; on exhaustion, forces a non-reserved first character.
...
## 2.1.0 — 2025-09-14
### Added
- Converted script to module; added `Export-ModuleMember -Function Get-RandomFileName`.
- `-MaxAttempts` parameter (default 100) to cap retries when avoiding reserved device names; on exhaustion, a safe first character is substituted.
### Fixed
- Documentation: first `.EXAMPLE` now includes the command; corrected `.PARAMETER MinimumLength` description.
...
Export-ModuleMember -Function Get-RandomFileName
