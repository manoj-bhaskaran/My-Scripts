...
- **[gdrive_recover] `--skip-existing` flag for local target collisions (gdrive_recover 1.25.0)** – `recover-and-download` now accepts `--skip-existing` as an alternative to `--overwrite`. When the computed local target path already exists on disk, the download is skipped, no bytes are written, and the item is still considered a successful operation: per-step state advances (`downloaded` is marked), the post-restore policy is still applied, and the skip is recorded against a new `stats["skipped_existing"]` counter. The two flags are mutually exclusive via an argparse `add_mutually_exclusive_group`; with neither flag set, the default behaviour is unchanged — a short uuid suffix is appended to the filename to produce a conflict-safe name. Implementation lives in `DriveDownloader.download` (skip branch with `stats_lock`-guarded counter increment) and `DriveTrashRecoveryTool._generate_target_path` (third condition that suppresses the uuid-rename when `--skip-existing` is active).
